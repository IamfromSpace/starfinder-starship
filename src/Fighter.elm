...
fighter model =
    ...
    let
        scale =
            "scale(" ++ String.fromFloat (model.size / 12800 / sqrt 2) ++ ")"

        translate =
            "translate(" ++ String.fromFloat (-12800 * (1 - sqrt 2) / 2) ++ "," ++ String.fromFloat (-12800 * (1 - sqrt 2) / 2) ++ ")"

        rotate =
            "rotate(-137.5," ++ String.fromFloat 6400 ++ "," ++ String.fromFloat 6400 ++ ")"
    in
    Svg.g
...
main =
    sandbox
        { init = { size = 400, color = Color.green }
